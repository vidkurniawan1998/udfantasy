$(document).ready(function () {
  ...
  if ($('.tinymce').length > 0) {
    tinymce.init({
      ...
      toolbar1:
        'insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image responsivefilemanager',
      ...
      external_filemanager_path:
        base_url + 'assets/template_admin/tinymce/plugins/filemanager/',
      filemanager_title: 'Responsive Filemanager',
      external_plugins: {
        'filemanager ':
          base_url +
          'assets/template_admin/tinymce/plugins/filemanager/plugin.min.js'
      }
    })
  }
    ...
    swal
      ...
      .then(function (result) {
        if (result.value) {
          ...
          swal.fire('Cancel', 'not removing the data', 'error')
        }
      })
  ...
  $('.datatable').on('click', '.btn-edit', function (e) {
    ...
    var json = $(this)
      .parents('td')
      .siblings('td.data-row')
      .children('textarea')
      .val()
    var json_image = $(this)
      .parents('td')
      .siblings('td.data-image-row')
      .children('textarea')
      .val()
    var json_item = $(this)
      .parents('td')
    ...
    $.each(data, function (field, value) {
      ...
      if (
        $('#modal-edit [name="' + field + '"]').hasClass('tinymce') &&
        !isEmpty(value)
      ) {
        ...
        $('#modal-edit [name="' + field + '"]').hasClass('browse-preview-img')
      ) {
        $('#modal-edit [name="' + field + '"]')
          .parents('div')
          .siblings('img')
          .attr('src', path_img + value)
      } else if (
        $('#modal-edit [name="' + field + '"]').attr('type') == 'checkbox'
      ) {
        if (value == 'yes') {
        ...
        $('#modal-edit [name="' + field + '"]').hasClass('input-select2')
      ) {
        ...
        $.each(value, function (array_key, array_value) {
          ...
          $.each(array_value, function (key, info_value) {
            if (array_key === 'information_name') {
              ...
              $('#modal-edit .product-info-' + (key + 1))
                .find('input[name="data_info[information_name][]"]')
                .val(info_value)
            } else if (array_key === 'information_value') {
              $('#modal-edit .product-info-' + (key + 1))
                .find('input[name="data_info[information_value][]"]')
                .val(info_value)
            }
          })
        })
    ...
    if (!isEmpty(json_image)) {
      ...
      $.each(data_image, function (key, row) {
        $.each(row, function (field, value) {
          if (
            $('#modal-edit [name="' + field + '[]"]').hasClass(
              'browse-preview-multi-img'
            )
          ) {
            if (
              !$('#modal-edit [name="' + field + '[]"]')
            ...
            $('#cloned-thumbnail-edit')
              .clone()
              .appendTo('#modal-edit .thumbnail-uploaded')
              .removeAttr('id')
              .removeClass('hidden')
              .addClass('img_' + strip_img_ext(value) + '_' + key)
              .children('img')
              .attr('src', path_img + value)
              .attr('width', '200')
            $('.img_' + strip_img_ext(value) + '_' + key)
              .children('#modal-edit .remove-thumbnail')
              .attr('data-remove', value)
            return false
          }
        })
      })
    ...
    if (!isEmpty(data_item)) {
      ...
      $('.data-item tbody').append(
        '<tr class="data-shipping-price">' +
          ...
          '<td>Rp. ' +
          addCommas(shipping_price) +
          '</td>' +
          '</tr>' +
          '<tr class="data-total-price">' +
          '<td colspan="4">Total</td>' +
          '<td>Rp. ' +
          addCommas(total_price) +
          '</td>' +
          '</tr>'
      )
    }
  ...
  $('.datatable').on('click', '.btn-edit-items', function (e) {
    ...
    $.each(data, function (field, value) {
      if ($('#modal-edit-items [name="' + field + '"]').hasClass('tinymce')) {
        ...
        $('#modal-edit-items [name="' + field + '"]').hasClass(
          'browse-preview-img'
        )
      ) {
        $('#modal-edit-items [name="' + field + '"]')
          .parents('div')
          .siblings('img')
          .attr('src', path_img + value)
      } else {
    ...
    if (!isEmpty(data_item)) {
      $.each(data_item, function (field, value) {
        $('#select-items')
          .clone()
          .appendTo('#modal-edit-items .div-items')
          .addClass('select-items-' + field)
          .addClass('items-all')
          .removeAttr('id')
          .removeClass('hidden')
        $('.select-items-' + field)
  ...
  $('.datatable').on('click', '.btn-detail', function (e) {
    ...
    $.each(data, function (field, value) {
      if (field == 'shipping_price') {
        ...
        $('.print-pdf').attr(
          'href',
          base_url + 'proweb/checkout/print_checkout/' + value
        )
      }
      if ($('#modal-detail [name="' + field + '"]').hasClass('tinymce')) {
        tinyMCE.activeEditor.setContent(value)
      } else if (
        $('#modal-detail [name="' + field + '"]').hasClass('browse-preview-img')
      ) {
        $('#modal-detail [name="' + field + '"]')
          .parents('div')
          .siblings('img')
          .attr('src', path_img + value)
      } else {
  ...
  function promotion_status_view(id_modal) {
    var status_seo_view = $('#' + id_modal)
      .find('input[name="promotion_status"]:checked')
      .val()
    if (status_seo_view === 'yes') {
      $('#' + id_modal)
        .find('.promotion-status-wrapper')
        .removeClass('hide')
    } else {
      $('#' + id_modal)
        .find('.promotion-status-wrapper')
  ...
  $('#modal-create').on(
    'change',
    'select[name="id_products_category"]',
    function () {
      selectedIdCategory = $('#modal-create')
        .find('select[name="id_products_category"]')
        .val()
  ...
  function find_sub_category(id_parent, id_modal) {
    $.ajax({
      ...
      url:
        base_url +
        '/proweb/products_sub_category/get_sub_category/' +
        selectedIdCategory,
      dataType: 'json',
      success: function (data) {
        var selectSubCategory = id_modal.find(
          'select[name="id_products_sub_category"]'
        )
        ...
        for (let index = 0; index < data.length; index++) {
          selectSiblings = selectSubCategory
            .siblings('input[name="id_products_sub_category"]')
            .val()
  ...
  $('.wrapper-filter-checkout').ready(function () {
    $('.wrapper-filter-checkout').on(
      'change',
      '#filter-checkout-district',
      function () {
    ...
    $('.wrapper-filter-checkout').on(
      'change',
      '#filter-checkout-payment-method',
      function () {
        filterColumn($(this).attr('data-column'))
      }
    )
...
function clone_row_product(modal) {
  ...
  var count_product_info = $(
    modal + ' input[name = "count_product_info"]'
  ).val()
  count_product_info++
  ...
  product_row
    .clone()
    .appendTo(container_info)
    .removeAttr('id')
    .removeClass('hidden')
    .addClass('row')
    .addClass('product-info-' + count_product_info)
  $(modal + ' .product-info-' + count_product_info)
    .find('.information-name')
    .attr('name', 'data_info[information_name][]')
  $(modal + ' .product-info-' + count_product_info)
    .find('.information-value')
    .attr('name', 'data_info[information_value][]')
  $(modal + ' .product-info-' + count_product_info)
    .find('.remove-info')
    .attr(
      'onclick',
      'remove_row_product("' + count_product_info + '","' + modal + '")'
    )
